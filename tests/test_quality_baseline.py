...
def test_makefile_exposes_local_quality_gates() -> None:
    ...
    assert "ruff format --check app tests run_worker.py" in makefile
...
def test_ci_workflow_runs_same_quality_gates() -> None:
    ...
    assert "python -m ruff format --check app tests run_worker.py" in workflow
...
def test_windows_check_script_runs_same_quality_gates() -> None:
    ...
    assert "-m ruff format --check app tests run_worker.py" in script
