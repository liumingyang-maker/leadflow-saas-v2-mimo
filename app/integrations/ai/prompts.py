...
def build_search_result_paste_parser_v2_prompt(
    *,
    ...
    system = (
        ...
        "or unsafe. Also include scoring_v2 for every candidate using 0-100 integer scores: "
        "buyer_score, product_fit_score, confidence_score, supplier_risk_score, "
        "directory_risk_score, marketplace_risk_score, safety_risk_score, positive_signals, "
        "risk_flags, classification_v2, score_band, and recommended_action. Do not output "
        "tiny raw confidence values like 7, 12, or 14 for clear buyer candidates unless they "
        "are truly low-confidence. Only buyer and maybe_buyer are candidate previews; supplier, "
        "directory, marketplace, article, irrelevant, and unsafe must be rejected or marked for "
        "discard. Packaging and LED can be scored more directly when product fit is clear. "
        "Hardware or OEM must be conservative. JSON keys must be exactly: "
        f"{', '.join(keys)}."
    )
    if locale == "zh-CN":
        system += " UI language is Simplified Chinese, but JSON keys stay English."
    user = (
        ...
        "match_reason, risk_reason, next_action, sanitized_snippet, and scoring_v2. "
        "Rejected items must include source_item_id and reason. Query feedback may include "
